...
/*   Updated: 2021/06/14 11:08:07 by mbeaujar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_texture(t_game *game, int i, int j, t_image image)
{
	...
	while (x < game->sq_lenx)
	{
		...
		{
			txt_x = image.width / (100 / ((double)((double)x / (double)game->sq_lenx) * 100.0));
			txt_y = image.height / (100 / ((double)((double)y / (double)game->sq_leny) * 100.0));
